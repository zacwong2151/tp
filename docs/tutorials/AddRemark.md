...
/**
 * Changes the remark of an existing volunteer in the address book.
 */
...
```java
    public static final String MESSAGE_USAGE = COMMAND_WORD
            + ": Edits the remark of the volunteer identified "
            + "by the index number used in the last volunteer listing. "
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the volunteer in the filtered volunteer list to edit the remark
     * @param remark of the volunteer to be updated to
     */
...
Now that we have all the information that we need, let’s lay the groundwork for propagating the remarks added into the in-memory storage of volunteer data. We achieve that by working with the `Person` model. Each field in a Person is implemented as a separate class (e.g. a `Name` object represents the volunteer’s name). That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a volunteer.

### Add a new `Remark` class

Create a new `Remark` in `seedu.address.model.volunteer`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each volunteer.

Simply add the following to [`seedu.address.ui.VolunteerCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-639834f1e05afe2276a86372adf0fe5f69314642c2d93cfa543d614ce5a76688).
...
```java
public PersonCard(Person volunteer, int displayedIndex) {
    //...
    remark.setText(volunteer.getRemark().value);
}
```
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person volunteerToEdit = lastShownList.get(index.getZeroBased());
        Person editedVolunteer = new Person(
                volunteerToEdit.getName(), volunteerToEdit.getPhone(), volunteerToEdit.getEmail(),
                volunteerToEdit.getAddress(), remark, volunteerToEdit.getTags());

        model.setPerson(volunteerToEdit, editedVolunteer);
        model.updateFilteredPersonList(PREDICATE_SHOW_ALL_PERSONS);

        return new CommandResult(generateSuccessMessage(editedVolunteer));
    }

    /**
     * Generates a command execution success message based on whether
     * the remark is added to or removed from
     * {@code volunteerToEdit}.
     */
    private String generateSuccessMessage(Person volunteerToEdit) {
        String message = !remark.value.isEmpty() ? MESSAGE_ADD_REMARK_SUCCESS : MESSAGE_DELETE_REMARK_SUCCESS;
        return String.format(message, volunteerToEdit);
    }
```
