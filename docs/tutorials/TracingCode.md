...
1. Now, step over until you read the statement that calls the `execute()` method of the `EditCommand` object received, and step into that `execute()` method (partial code given below):
   ...
   **`EditCommand#execute()`:**
   ...
   public CommandResult execute(Model model) throws CommandException {
       ...
       Person editedVolunteer = createEditedPerson(volunteerToEdit, editPersonDescriptor);
       ...
       return new CommandResult(String.format(MESSAGE_EDIT_PERSON_SUCCESS, editedVolunteer));
   }
   ```

1. As suspected, `command#execute()` does indeed make changes to the `model` object. Specifically,
   * it uses the `setPerson()` method (defined in the interface `Model` and implemented in `ModelManager` as per the usual pattern) to update the volunteer data.
   * it uses the `updateFilteredPersonList` method to ask the `Model` to populate the 'filtered list' with _all_ volunteers.<br>
     FYI, The 'filtered list' is the list of volunteers resulting from the most recent operation that will be shown to the user immediately after. For the `edit` command, we populate it with all the volunteers so that the user can see the edited volunteer along with all other volunteers. If this was a `find` command, we would be setting that list to contain the search results instead.<br>
     To provide some context, given below is the class diagram of the `Model` component. See if you can figure out where the 'filtered list' of volunteers is being tracked.
